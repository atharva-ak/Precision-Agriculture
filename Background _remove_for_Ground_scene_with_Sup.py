###### Background remove for Ground scene with Super-pixel ###
import skimage.segmentation as seg
from skimage import color
import cv2
import numpy as np
img = cv2.imread('./defect_2/DJI_0180_mod2.JPG')
rows = img.shape[0]
cols = img.shape[1]
rows = int((rows*80)/100)
cols = int((cols*80)/100)
img = cv2.resize(img, (cols,rows)) 
image_slic = seg.slic(img,n_segments=100)
img_out1 = img.copy()
img_out2 =img.copy()
# label2rgb replaces each discrete label with the average interior 
colorimage_show = color.label2rgb(image_slic, img, kind='avg')

image_slic2 = seg.slic(colorimage_show,n_segments=20)
colorimage_show2 = color.label2rgb(image_slic2, img, kind='avg')

image_slic3 = seg.slic(colorimage_show2,n_segments=10)
colorimage_show3 = color.label2rgb(image_slic3, img, kind='avg')
# gray = cv2.cvtColor(colorimage_show, cv2.COLOR_BGR2GRAY)
r = colorimage_show3[:,:,2]
# r1, thresh = cv2.threshold(r,170,255,cv2.THRESH_BINARY_INV)
ret4,thresh = cv2.threshold(r,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)

edges = cv2.Canny(thresh,0,255)
contours,hier = cv2.findContours(edges,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
filled = cv2.drawContours(thresh,contours,-1,(255,255,255),15)


cv2.imshow("Normal", colorimage_show)
cv2.imshow("Defected", colorimage_show2)
cv2.imshow("rrr", colorimage_show3)
